...
SummaryController.preFlight = async (instance, userData) => {
  ...
  if (performsSubmit) {
    const currrentUrl = userData.req.originalUrl // getUrl(pageInstance._id)

    let parentInstance = pageInstance
    while (parentInstance._parent) {
      parentInstance = getInstance(pageInstance._parent)
    }
    // Check where we get if we replay from intitial page
    let reachedPageInstance = deepClone(getInstance(parentInstance._id))
