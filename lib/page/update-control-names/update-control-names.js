...
const getRedactedValue = (nameInstance, userData, skipRedaction, scope) => {
  // Would make more sense to use scope?
  let value = userData.getUserDataProperty(`${nameInstance.name}__ANSWER`, undefined, scope)
  if (value === undefined) {
    value = userData.getUserDataProperty(nameInstance.name, undefined, scope)
  }
