...
const checkNextInputPage = (inputPage, userData) => {
  const {_id} = inputPage
  const userParams = userData.getUserParams() || {}
  const params = userParams.id ? userParams : inputPage.params
...
// Gash - but will do for now
// TODO: make this recognise non-repeatable params
// thought possibly should be handled by
const fixPageParams = (page, params = {}) => {
  if (params.id) {
...
const getNextPage = (inputPage, userData, recurse) => {
  ...
  if (checkPageVisibility(nextPage, userData)) {
    return fixPageParams(nextPage, params)
  } else {
...
const getPreviousUrl = (inputPage, userData) => {
  let previouspage = getPreviousPage(inputPage, userData)
  while (previouspage && getInstance(previouspage._id).skip) {
    previouspage = getPreviousPage(previouspage, userData)
  }
  if (previouspage) {
...
// }
// TODO: handle non-repeatable params properly (ie. at all)
