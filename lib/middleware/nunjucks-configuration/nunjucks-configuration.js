...
const fbNunjucksHelpers = require('@solidgoldpig/fb-nunjucks-helpers')
...
const nunjucksConfiguration = (app, components = [], options = {}) => {
  ...
  const addSearchPaths = (basePath) => {
    ...
    // ensure templates can be reached with explicit full paths
    const subBasePath = basePath.replace(/\/[^/]+$/, '')
    addSearchPath(subBasePath)
  }
  ...
  // Add macros
  const addMacroPaths = (...globs) => {
    globs.forEach(macrosGlob => {
  ...
  const addMacros = (basePath) => {
    ...
    if (fbStyle) {
      addMacroPaths(`${specsPath}/**/*.njk`, `${componentsPath}/**/*.njk`)
    } else {
      addMacroPaths(`${basePath}/**/macro.njk`)
    }
  }
...
const getPageOutput = async (pageInstance, context = {}) => {
  const rendered = new Promise((resolve, reject) => {
    ...
    const templatePrefix = `${page.EDITMODE === 'edit' ? 'fb-editor-node' : 'fb-components-core'}/templates/nunjucks/`
    const templatePaths = {
      page: `${templatePrefix}page.njk.html`
    }
    const renderContext = Object.assign({}, context, {page, templatePaths})
