...
const {FBLogger, deepClone} = require('@solidgoldpig/fb-utils-node')
...
external.getUserDataMethods = (data = {}, params = {}, additional = {}) => {
  ...
  const getDataScope = (dataScope) => {
    const actualScope = dataScope || scope
    data[actualScope] = data[actualScope] || {}
    return data[actualScope]
  }
  ...
  const bundle = {
    ...
    setUserData: (updatedData, dataScope, replace) => {
      const actualScope = dataScope || scope
      const newData = replace ? updatedData : Object.assign({}, getDataScope(actualScope), updatedData)
      data[actualScope] = newData
      return newData
    },
...
external.loadUserData = async (req, res) => {
  ...
  // Allows saveData to be redefined for now
